...
export function BrowserShell() {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      const api = bridge()
      const query = api?.searchHistory ? api.searchHistory(address, 6) : Promise.resolve(state.history)
  ...
  }, [suggestions, address, state.bookmarks, state.history])
  useEffect(() => {
