  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{ color: 'rgba(var(--primary-rgb), 0.4)', letterSpacing: '0.1em' }}
      >
        {title}
      </div>
      ...
      {items.length > 0 ? (
        <div className="flex flex-col">
          {items.slice(0, 5).map((item, i) => (
            <button type="button"
              ...
              style={{
                borderBottom: i < Math.min(items.length, 5) - 1 ? '1px solid rgba(var(--primary-rgb), 0.05)' : 'none',
              }}
            >
              {item.favicon ? (
              ...
              <span className="font-body text-xs truncate" style={{ color: 'rgba(var(--neutral-rgb), 0.6)' }}>
                {item.title}
              </span>
            </button>
          ))}
        ...
        <div className="py-4 text-center">
          <p className="font-body text-[11px]" style={{ color: 'rgba(var(--neutral-rgb), 0.3)' }}>{emptyMessage}</p>
        </div>
      )}
    </div>
  )
