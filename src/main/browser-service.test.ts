...
describe('BrowserService native state ownership', () => {
  let dir: string
  const window = { addBrowserView: vi.fn(), removeBrowserView: vi.fn(), getContentSize: () => [1200, 800], on: vi.fn(), isDestroyed: () => false, setBackgroundColor: vi.fn(), setTitleBarOverlay: vi.fn(), webContents: { send: vi.fn(), isDestroyed: () => false, id: 1 } }
